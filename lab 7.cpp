...
void printAccounts(bankAccount **arr,int);
bool makeTransaction(bankAccount ** arr, int Size,int ID, float Amount, bool TransType);
...
//	/*Write your code here for task 2, ~6 lines*/
	b1->print();
	b1->deposit(1000);//successful as the previous balance was 13000 and balance limit is 15000 so 1000 will be added
	b1->deposit(3000);//unsuccessful as it will exceed limit
	b1->withdraw(10000);//sucessfull 
	b1->withdraw(6000);//unsuceesful not enough balance
	b1->print();
//	//***************************END OF TASK 2**********************************
//	
//
...
//	//*****Write your code here
	const int n=2;
	bankAccount *ptrarr[n];
	float trans[3]={3000,5000,60000};
	ptrarr[0]=new bankAccount(30,2000,trans,3);//2 constructor
	ptrarr[1]=new bankAccount(40,255000,400000);//1st constructor
	ptrarr[0]->print();
	ptrarr[1]->print();
...
//	//*****Write your code here
//	*/
	bankAccount **dyptrarr=new bankAccount *[2];
	float trans2[2]={-5000,5000};
	dyptrarr[0]=new bankAccount(50,2000,trans2,2);
	dyptrarr[1]=new bankAccount(60,6000,400000);
	dyptrarr[0]->withdraw(1000);
	dyptrarr[0]->print();
	dyptrarr[1]->print();
...
//	*/
	bool type=0;
   makeTransaction(arr,size,5,6000,type); 
...
bankAccount ** readFile(string filename,int &size)
{
	int totalacconts=0,id=0,nooftrans=0;
	float *tran;

	float balance=0,limit=0;
	ifstream fin;
	fin.open(filename);
	fin>>totalacconts;
	bankAccount **arr=new bankAccount*[totalacconts];
	for(int i=0;i<totalacconts;i++)
		{
			fin>>id;
		...
		fin>>nooftrans;
		tran=new float [nooftrans];
		for(int i=0;i<nooftrans;i++)
			{
				fin>>tran[i];
			
			}
		arr[i]=new bankAccount(id,balance,tran,nooftrans,limit);
...
bool makeTransaction(bankAccount ** arr, int size,int ID, float Amount, bool type)
{
	bool flag=0;
	for(int i=0;i<size;i++)
	{
		if(arr[i]->getID()==ID)
		{
			if(type==0)//checks weather it is withdraw or not
			{
...
void deleteAccounts(bankAccount **arr,int size)
{
	for (int c=0;c<size;++c){
		if (arr[c]->gettrans()==0 && arr[c]->getbalance()==0)
		{
			delete arr[c];
			arr[c]=nullptr;
		}
